...
class RecordDialog(QDialog):
    ...
    def __init__(self, record: ContactRecord | None, parent: QWidget | None = None, *, record_id: str | None = None):
        """Build the form pre-filled with the record's current values.

        Args:
            record: Contact being edited, or `None` when adding.
            parent: Owning window.
            record_id: Identifier assigned to a new record.
        """
        super().__init__(parent)
        self._record_id = record.id if record is not None else record_id
        if self._record_id is None:
            raise ValueError("record_id is required when adding a record")
        labels = {field: per_app[_APP_KEY] for field, per_app in FIELD_LABELS.items()}
        self.setWindowTitle(f"Edit Record — {record.full_name}" if record is not None else "Add Record")
...
class CrmBWindow(QMainWindow):
    ...
    def add_record(self) -> None:
        """Open a blank modal and persist only after its explicit Add Record action."""
    ...
    def apply_create(self, record: ContactRecord) -> None:
        """Append and persist one explicitly confirmed new record.

        Args:
            record: Validated contact returned by the add dialog.
        """
    ...
    def _build_ui(self) -> None:
        ...
        self.open_button = QPushButton("Edit Record…")
